...
window.initEventsBrutalist = function () {
        ...
        topEvents.forEach((ev, i) => {
            ...
            block.innerHTML = `
                <div class="h-img-wrap w-full md:w-[45%] h-[30vh] sm:h-[40vh] md:h-[60vh] relative overflow-hidden rounded-[16px] sm:rounded-[20px] md:rounded-[40px] border border-white/10 z-10" data-cursor-hover>
                    <img src="${ev.img}" class="h-img w-full h-[120%] top-[-10%] left-0 absolute object-cover" alt="${ev.title}" loading="lazy">
    ...
    if (pastTrack) {
        ...
        pastEvents.forEach((ev, i) => {
            ...
            card.innerHTML = `
                <img src="${ev.img}" class="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-110 transition-all duration-700 ease-out filter grayscale-[50%] group-hover:grayscale-0" loading="lazy">
